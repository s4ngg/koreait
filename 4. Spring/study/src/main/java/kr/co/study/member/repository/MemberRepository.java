...
/**
 * 아이디 중복 체크
 * - Jpa 메서드 이름 규칙에 따라 자동으로 쿼리가 생성된다.
 * - SELECT count(*) > 0 FROM member WHERE userId = ?
 * 
 * @param userId 사용자가 회원가입에서 입력한 아이디
 * @return true : 이미 해당 사용자가 userId를 사용 중
 * 		   false : 존재하지 않음 (사용 가능)
 */
 ...
 * 
 * @param userId 사용자가 회원가입에서 입력한 이메일
 * @return true : 이미 해당 사용자가 email을 사용 중
 * 		   false : 존재하지 않음 (사용 가능)
 */	
 ...
 *
 * 참고
 * 	- 여러 개의 SELECT 결과를 받을 경우 List가 사용된다.
